...
symhash_add(D_SymHash *sh, D_Sym *s) {
  ...
  if (sh->index > sh->grow) {
    ...
    sh->syms.n = sh->grow;
    sh->grow = sh->grow * 2 + 1;
    sh->syms.v = reinterpret_cast<D_Sym**>((MALLOC(sh->syms.n * sizeof(void *))));
...
static D_SymHash * 
new_D_SymHash() {
  D_SymHash *sh = reinterpret_cast<D_SymHash*>(MALLOC(sizeof(D_SymHash)));
...
new_D_Scope(D_Scope *parent) {
  D_Scope *st = reinterpret_cast<D_Scope*>(MALLOC(sizeof(D_Scope)));
...
enter_D_Scope(D_Scope *current, D_Scope *scope) {
  D_Scope *st = reinterpret_cast<D_Scope*>(MALLOC(sizeof(D_Scope))), *parent = scope->up;
  memset(st, 0, sizeof(D_Scope));
...
scope_D_Scope(D_Scope *current, D_Scope *scope) {
  D_Scope *st = reinterpret_cast<D_Scope*>(MALLOC(sizeof(D_Scope))), *parent = current->up;
  memset(st, 0, sizeof(D_Scope));
...
new_D_Sym(D_Scope *st, char *name, char *end, int sizeof_D_Sym) {
  ...
  D_Sym *s = reinterpret_cast<D_Sym*>(MALLOC(sizeof_D_Sym));
