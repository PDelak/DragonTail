...
std::ostream &operator<<(std::ostream &stream,
                         const BasicExpression &expression) {
  stream << expression.value;
  return stream;
}

std::ostream &operator<<(std::ostream &stream, const Expression &expression) {
  std::for_each(expression.child_begin(), expression.child_end(),
                [&](const StatementPtr &stmt) {
                  static_cast<Statement *>(stmt.get())->text(stream);
                });
  if (!expression.isPartOfCompoundStmt)
    stream << ";\n";
  return stream;
}
...
std::ostream &operator<<(std::ostream &stream, const IfStatement &ifStatement) {
  ...
  stream << ") ";
  std::for_each(ifStatement.statements.begin(), ifStatement.statements.end(),
                [&](const StatementPtr &stmt) { stmt->text(stream); });
  return stream;
}
...
std::ostream &operator<<(std::ostream &stream, const WhileLoop &loop) {
  ...
  stream << ") ";
  std::for_each(loop.statements.begin(), loop.statements.end(),
                [&](const StatementPtr &stmt) { stmt->text(stream); });
  return stream;
}
...
std::ostream &operator<<(std::ostream &stream,
  ...
  stream << "(";
  std::for_each(functionCall.parameters.begin(), functionCall.parameters.end(),
...
std::ostream &operator<<(std::ostream &stream,
  ...
  stream << "(";
  std::for_each(functionDecl.parameters.begin(), functionDecl.parameters.end(),
                [&](const std::string &param) {
                  if (first) {
                    stream << " ";
                  }
