...
struct CFGFlattener : public AstVisitor {
  CFGFlattener() {
    ...
    statements.push_back(makeNode(
        Expression(scope, {makeNode(BasicExpression(scope, "__alloc__"))})));
  }
  ...
  void visitPre(const Expression *stmt) {
    nodesStack.push_back(makeNode(
        Expression(scope, stmt->getChilds(), stmt->isPartOfCompoundStmt)));
  }
  ...
  void visitPre(const FunctionDecl *fdecl) {
    ...
    nodesStack.push_back(makeNode(FunctionDecl(
        scope, fdecl->name, fdecl->parameters, fdecl->statements)));
  }
  ...
  void visitPost(const IfStatement *) {
    ...
    if (is<BlockStatement>(*currentStatementIterator)) {
      ...
      std::for_each(
          ...
          [](const StatementPtr &stmt) { stmt->scope = stmt->scope - 1; });
      std::copy(block->statements.begin(), block->statements.end(),
                std::back_inserter(if_children));
    } else {
    ...
    condition.setElements(
        cast<Expression>(*currentStatementIterator)->getChilds());
    cast<IfStatement>(if_statement)->condition.isPartOfCompoundStmt =
        cast<Expression>(*currentStatementIterator)->isPartOfCompoundStmt;
    ++currentStatementIterator;
    ...
    auto reverseCondition =
        makeNode(Expression(scope, {makeNode(BasicExpression(scope, temp)),
                                    makeNode(BasicExpression(scope, "="))}));

    std::copy(
        condition.child_begin(), condition.child_end(),
        std::back_inserter(
            static_cast<Expression *>(reverseCondition.get())->getChilds()));
    statements.push_back(reverseCondition);

    cast<IfStatement>(if_statement)
        ->condition.insertChild(
            cast<IfStatement>(if_statement)->condition.child_end(),
            {makeNode(BasicExpression(scope, "!")),
    ...
    cast<IfStatement>(if_statement)
        ->statements.push_back(
            makeNode(GotoStatement(gotoStatementScope, label)));
    statements.push_back(if_statement);

    std::copy(if_children.begin(), if_children.end(),
  ...
  void visitPost(const WhileLoop *) {
    ...
    */
    ...
    if (is<BlockStatement>(*currentStatementIterator)) {
      ...
      // reducing while loop means that it is transformed to if statement with
      // reversed condition and few jumps depending on condition value so next
      // for_each statement is responsible for decreasing scope counter of all
      // statements inside a block
      std::for_each(
          block->statements.begin(), block->statements.end(),
          [](const StatementPtr &stmt) { stmt->scope = stmt->scope - 1; });
      std::copy(block->statements.begin(), block->statements.end(),
                std::back_inserter(while_children));
    } else {
      while_children.push_back(*currentStatementIterator);
    }
    ...
    cast<IfStatement>(if_statement)->scope =
        cast<Expression>(*currentStatementIterator)->scope;
    cast<IfStatement>(if_statement)->condition.isPartOfCompoundStmt =
        cast<Expression>(*currentStatementIterator)->isPartOfCompoundStmt;
    ...
    statements.push_back(if_statement);
    std::copy(while_children.begin(), while_children.end(),
              std::back_inserter(statements));
    statements.push_back(makeNode(
        GotoStatement(scope, cast<LabelStatement>(labelBeforeIfNode)->label)));
    statements.push_back(makeNode(LabelStatement(scope, label)));
  }
  void visitPost(const BlockStatement *) {
    ...
    if (previous != nodesStack.begin())
      --previous;
    if (!nodesStack.empty() &&
        (is<IfStatement>(*previous) || is<WhileLoop>(*previous))) {
      StatementList blockStatements;
      auto it = std::find_if(
          statements.rbegin(), statements.rend(),
          [&](const StatementPtr &s) { return s->scope != scope; });

      // insert label just before body of compound statement (if, while loop)
      blockStatements.push_back(makeNode(
          Expression(scope, {makeNode(BasicExpression(scope, "__alloc__"))})));

      std::copy(it.base(), statements.end(),
                std::back_inserter(blockStatements));
      blockStatements.push_back(makeNode(Expression(
          scope, {makeNode(BasicExpression(scope, "__dealloc__"))})));
      ...
      // std::string label = getNextLabel();
      // auto first_statement_after_label = statements.insert(it.base(),
      // makeNode(LabelStatement(scope, label)));
  ...
  void visitPost(const VarDecl *) { popFromStackAndPushToStatements(); }

  void visitPost(const Expression *) { popFromStackAndPushToStatements(); }

  void visitPost(const LabelStatement *) { popFromStackAndPushToStatements(); }

  void visitPost(const GotoStatement *) { popFromStackAndPushToStatements(); }

  void visitPost(const ReturnStatement *) { popFromStackAndPushToStatements(); }
  ...
  void visitPost(const FunctionDecl *) {
    ...
    static_cast<LabelStatement *>(label.get())->label =
        static_cast<FunctionDecl *>(node.get())->name;
    ...
    for (const auto &stmt :
         static_cast<FunctionDecl *>(node.get())->statements) {
      stmt->traverse(*this);
    }
  ...
  StatementList getStatements() {
    ...
    if (!closingDealloc) {
      statements.push_back(makeNode(Expression(
          scope, {makeNode(BasicExpression(scope, "__dealloc__"))})));
      closingDealloc = true;
    }
    return statements;
